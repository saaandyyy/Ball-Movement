...
function keyCode(keyPreass) {
    var top = parseInt(ball.style.top);
    var left = parseInt(ball.style.left);

    // W is pressed
    if(keyPreass === 119 || keyPreass === 87) {
        if (top > 5) {
    ...
    // A is Pressed
    else if (keyPreass === 97 || keyPreass === 65) {
        if (left > 5) {
    ...
    // S is Pressed
    else if (keyPreass === 115 || keyPreass === 83) {
    ...
    // D is Pressed
    else if (keyPreass === 100 || keyPreass === 68) {
...
window.addEventListener("keypress", function (event) {
    // keyCode(event.code); // You can use this
